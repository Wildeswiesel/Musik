...
def get_audio_metadata(file_path):
    ...
    metadata["Year"] = audio.get("date", ["Unknown"])[0]  # 'date' for year
...
def analyze_audio(file_path):
    ...
    legend_elements = [
        plt.Line2D([0], [0], color='green', lw=4, label='Quiet'),
        plt.Line2D([0], [0], color='purple', lw=4, label='Medium'),
        plt.Line2D([0], [0], color='red', lw=4, label='Loud')
    ]
    plt.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0.9, 0.8), fontsize=12)
